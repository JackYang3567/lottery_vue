<template>
  <div id="app">
      <transition name="for-height-change">
          <div class="for-show-content-history" v-if="show_history_div && !getRoom" style="margin-top: 15px;" @click="closeThisDiv()">
              <div class="promptbox-wrapper prompt-in-game-rule sd115 prompt-gamerecords-official open-xiaoguo" style="position:static !important;">
                  <section class="promptbox" style="height: 100%;">
                      <div class="prompt-body prompt-body--gamerecords" style="height: 100%;">
                                      ...
                                      <tr v-for="v in dayHistory">
                                          ...
                                          <td class="primary-color" style="text-align: center;">
                                              <template v-for="(ball_number,k) in v['content']">
                                                  <span class="this-is-a-jiahao" v-if="k==6">＋</span>
                                                  <div style="display: inline-block;margin-right:8px;">
                                                      <span  class="this-is-a-ball" :style="'font-size: 0.75rem;display: block;background:'+ball_number[1]">{{ball_number[0]}}</span>
                                                      <span :style="'color:'+ball_number[1]">{{ball_number[2]}}</span>
                                                  </div>
                                              </template>
                                          </td>
                                      </tr>
                          ...
                          <template v-else>
                              <div style="width: 100%;height: 100%;text-align: center;background: #fff;padding-top: 25%;">
                                  暂无开奖记录
                              </div>
                          </template>
                      </div>
    ...
    <main :class="['kd-body game game-l has-user-playing-info',{'cart-not-empty':Object.keys(select_data.betting_all).length}]">
        ...
        <div class="lottery-wrapper" style="border-bottom: none;">

            <div style="display: flex;width: 100%;max-width: 640px;">
                <div style="width: 86%;height: 78px;box-sizing:content-box;">
                    ...
                    <div class="show-code-for-expect">
                        <template v-if="is_feng">
                            ...
                            <span data-v-7ce3e059="" class="spansAll" v-for="i in ['封','盘','中']">{{i}}</span>
                        </template>
                        <template v-else>
                            <template v-if="history.content">
                                <ul class="nostyle kd-row prev-lottery-balls">
                                    <li v-for="(item,key) in (history.content ? history.content : 0)" v-if="key < 6">
                                    ...
                                    <li v-for="(item,key) in (history.content ? history.content : 0)" v-if="key > 5">
                                        <span :class="'lottery-ball solid marksix ' + $parent.codeType(item.code)">{{ item.code }}</span><br><span class="lottery-prop">{{ item.type }}</span>
                                    </li>
                                </ul>
                            </template>
                            <template v-else>
                                <span data-v-7ce3e059="" class="spansAll" v-for="i in ['正','在','开','奖']">{{i}}</span>
                            </template>
                        </template>
                    </div>
                </div>
                <div class="show-left" style="width: 40%;height: 78px;box-sizing:content-box;text-align: center;">
        ...
        <div class="playing-methods-wrapper clearfix" :style="{top:(top_height+127)+'px'}">
            ...
            if(res.code){
              ...
              _this.timeTool();
                ...
                if(res.data.history['content'].length > 1){
                    ...
                    _this.history['content'] = res.data.history['content'];
                }
              _this.loading = false;
            }else{
...
<style scoped>
    .kd-body {
        padding-top: 7.725rem !important;
    }
    .kd-buttons{
        padding-top: 10px;
