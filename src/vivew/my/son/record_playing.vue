          ...
          <template v-for="(item,key) in data_sel">
            <li class="item kd-box data"  :style="{ borderTop:'3px solid ' + $store.state.basic.main_color }">
                ...
                <div v-else class="data-item">
                    <span class="data-item-key">期号：</span>
                    <span class="data-item-value">{{ item.expect }}</span>
                </div>
                <div v-if="buy==0">
                    <span class="for-ce-dan" v-if="item.state==0 && show_chedan.indexOf(item.id)>-1" @click="cancelTheOrder(item.id,item.user_id,$event)
...
<script>
  ...
  export default{
    ...
    methods:{
        //撤单方法
     cancelTheOrder(id,user_id,$event)
     {
         var _this = this;
         _this.$getData({
             type:'post',
             url:'/api/home/my/cancelTheOrder',
             data:{id:id,user_id:user_id},
             callback:function(res){
                 if(res == 1)
                 {
                     var key = _this.show_chedan.indexOf(id);
                     if(key != -1)
                     {
                         _this.show_chedan.splice(key,1);
                     }
                     _this.$total({ message:'撤单成功！',type: 1 });
                 }
                 else
                 {
                     _this.$total({ message:res,type: 2 });
                 }
             }
         });
     },
          ...
          callback:function(res){
              ...
              for (var i=0;i<res.data.length;i++)
              {
                  if(res.data[i].state==0){
                      _this.show_chedan.push(res.data[i].id);
                  }
              }
